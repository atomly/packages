...
export class Database {
  ...
  public async deleteEntities(): Promise<void> {
    ...
    // the name match the 'atomly_sandbox_test' pattern.
    if (
      (
        databaseHost === 'localhost' ||
        databaseHost === '192.168.99.101'
      ) &&
      databaseName === 'atomly_sandbox_test'
    ) {
      ...
      // eslint-disable-next-line no-console
      console.warn('DEBUG: Only the *atomly_sandbox_test* database can be dropped through TypeORM.');
    }
  }
}
