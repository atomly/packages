...
export class DefaultEntityLoadersFactory<
  E extends TEnum = string, // Properties of T.
  T = unknown,// T, instance of C.
  O extends DataAdapterOptions<T> = DataAdapterOptions<T>,
  C extends new() => T = new() => T, // Class of T.
> implements EntityLoadersFactory<T, E> {
  ...
  private dataAdapter: DataAdapter<T, C, O>;

  private entityReferenceIdKeys: Record<string, E>;

  private entityReferenceIdKeysList: E[];

  private entityReferenceIdKeysParams?: Partial<Record<E, O>>;

  constructor(args: {
    entity: C,
    dataAdapter: DataAdapter<T, C, O>,
    entityReferenceIdKeys: Record<string, E>;
    entityReferenceIdKeysParams?: Partial<Record<E, O>>;
  }) {
    // Setting up private stuff:
    this.entity = args.entity;
    this.dataAdapter = args.dataAdapter;
    this.entityReferenceIdKeys = args.entityReferenceIdKeys;
    this.entityReferenceIdKeysList = Object.values(this.entityReferenceIdKeys);
  ...
  private setupDataLoaders(referenceEntityIdKey: E, cacheMaps: ICacheMaps<T>): IDataLoaders<T> {
    const params = this.entityReferenceIdKeysParams && this.entityReferenceIdKeysParams[referenceEntityIdKey as E];
    // TODO: Improve how this is built.
    // Setting up DataLoader configurations:
    const oneConfig: O['oneToOneConfig'] = {
      batchFunctionConfig: {
        ...(params?.oneToOneConfig ?? DEFAULT_FACTORY_LOADER_CONFIG).batchFunctionConfig,
        entityIdKey: String(referenceEntityIdKey),
      },
      dataLoaderOptions: {
        ...(params?.oneToOneConfig ?? DEFAULT_FACTORY_LOADER_CONFIG).dataLoaderOptions,
        cacheMap: cacheMaps.one,
      },
    };
    const manyConfig: O['oneToManyConfig'] = {
      batchFunctionConfig: {
        ...(params?.oneToManyConfig ?? DEFAULT_FACTORY_LOADER_CONFIG).batchFunctionConfig,
        entityIdKey: String(referenceEntityIdKey), 
      },
      dataLoaderOptions: {
        ...(params?.oneToManyConfig ?? DEFAULT_FACTORY_LOADER_CONFIG).dataLoaderOptions,
        cacheMap: cacheMaps.many,
      },
    };
    const manyLimitedConfig: O['oneToManyLimitedConfig'] = {
      batchFunctionConfig: {
        ...(params?.oneToManyLimitedConfig ?? DEFAULT_FACTORY_LOADER_CONFIG).batchFunctionConfig,
        entityIdKey: String(referenceEntityIdKey),
      },
      dataLoaderOptions: {
        ...(params?.oneToManyLimitedConfig ?? DEFAULT_FACTORY_LOADER_CONFIG).dataLoaderOptions,
        cacheMap: cacheMaps.manyLimited,
      },
    };
    // Setting up DataLoaders:
    const dataLoaders: IDataLoaders<T> = {
      one: this.dataAdapter.oneToOneDataLoaderFactory(this.entity, oneConfig),
      many: this.dataAdapter.oneToManyDataLoaderFactory(this.entity, manyConfig),
      manyLimited: this.dataAdapter.oneToManyLimitedDataLoaderFactory(this.entity, manyLimitedConfig),
    };
    return dataLoaders;
  }
    ...
    if (entity) {
      for (const entityReferenceIdKey of this.entityReferenceIdKeysList) {
        if (
          ...
          const alternativeKeyValue = String(entity[entityReferenceIdKey as unknown as keyof T] as unknown);
