...
export interface IUpdaters<T extends BaseEntity> {
  one: (key: string) => Promise<T>
  many: (key: string) => Promise<T[]>
  manyLimited: (key: string) => Promise<T[]>
}
...
// export interface ICacheMapsBy<T extends BaseEntity> {
//   [key: string]: {
