  ...
  private async models(): Promise<void> {
    try {
      const result = await new Promise((res, rej) => {
        ...
        const cmd = `typeorm-model-generator -h ${this.connection?.host as string} -p ${this.connection?.port as string} -d ${this.connection?.database} -u ${this.connection?.username as string} -x ${this.connection?.password as string} -e ${this.connection?.type} -s ${this.connection?.schema as string} --eol LF --noConfig -o ${outputPath}`;
