...
3. Start a docker instance of the postgres image.
    - `sudo docker run --name beast-redis-0 -e -d -p 6379:6379 redis`
      - The name of the container is `beast-redis-0`.
...
4. Bash into the redis instance with `sudo docker exec -it beast-redis-0 redis-cli`.
...
2. Start the container with `sudo docker start beast-redis-0`.
3. Bash into the postgres instance with `sudo docker exec -it beast-redis-0 bash`.

## Troubleshooting

- Try restaring the container with `sudo docker restart beast-redis-0`;
- Try stopping the container with `sudo docker stop beast-redis-0`, then re-run the container with `sudo docker start beast-redis-0`;
